...
def get_output_path(args):
    folder_name = args.geneExpression_file_name.split('.csv')[0] + '-' + args.module_source + '-' + args.experiment_name + '_0000'
    output_dir = args.output_dir + folder_name + '/'
    
    if os.path.exists(output_dir):
        filter_str = args.geneExpression_file_name.split('.csv')[0] + '-' + args.module_source + '-' + args.experiment_name
        max_num = -1
        for folder_i in os.listdir(args.output_dir):
            if folder_i.startswith('.'): continue
            if filter_str in folder_i:
                num = int(folder_i.split('-')[-1].split(args.experiment_name)[-1].split('_')[1])
                max_num = max(max_num, num)
        max_num += 1
        folder_name = args.geneExpression_file_name.split('.csv')[0] + '-' + args.module_source + '-' + args.experiment_name + '_' + str(max_num).zfill(4)
        output_dir = args.output_dir + folder_name + '/'
...
def remove_non_connected_graph(compounds_flux):
    ...
    keep_nodes = list(largest_connected_component.intersection(set(compounds_flux.columns.values)))
    keep_factors = list(largest_connected_component.intersection(set(compounds_flux.index.values)))
...
def update_compoundsModules_modulesGenes(compounds_modules, modules_genes, geneExpression):
...
def perturb_modules(samples_modules, module_info, sm_id):
    ...
    if 'SM_id' not in module_info.columns.values:
        ...
        rdm_idx = np.random.choice(samples_modules.shape[1], int(samples_modules.shape[1]/2))[0]
    ...
    modules_intersection = list(set(modules).intersection(set(samples_modules.columns.values.tolist())))
    modules_intersection = list(set(modules_intersection))
    modules_intersection.sort()
    print(modules_intersection)

    samples_modules[modules_intersection] = samples_modules[modules_intersection] * times
    return samples_modules
...
def intersect_samples_genes(geneExpression, modules_genes):
    ...
    for module_i, genes in module_genes_new.items():
        ...
        if len(cur_intersect_genes) != 0:
            module_genes_new[module_i] = cur_intersect_genes
        print("\n Cur module_i:{0}, intersection of genes:{1}".format(module_i, cur_intersect_genes))
...
def check_intersect_genes(geneExpression_genes, modules_genes):
    ...
    for module_i, genes in modules_genes.items():
        ...
        print("\n Cur module_i:{0}, intersection of genes:{1}".format(module_i, cur_genes_intersection))
...
def get_std_scale_imbalanceLoss_realData(compounds_modules, samples_modules_scfea, samples_modules_mpo,
                                         samples_modules_snn,samples_modules_mposnn):
    '''
    ...
    '''
    times = 1000.0
    samples_modules_scfea = samples_modules_scfea.div(samples_modules_scfea.sum(axis=1), axis=0) * times
    samples_modules_mpo = samples_modules_mpo.div(samples_modules_mpo.sum(axis=1), axis=0) * times
    samples_modules_snn = samples_modules_snn.div(samples_modules_snn.sum(axis=1), axis=0) * times
    samples_modules_mposnn = samples_modules_mposnn.div(samples_modules_mposnn.sum(axis=1), axis=0) * times
    ...
    imbalanceLoss_mean_mposnn = np.mean(get_imbalanceLoss(compounds_modules, samples_modules_mposnn.values))
    ...
    res = {'std_mean_col_scfea': std_mean_col_scfea, 'std_mean_col_mpo': std_mean_col_mpo,
           'std_mean_col_snn': std_mean_col_snn,'std_mean_col_mposnn': std_mean_col_mposnn,
           'imbalanceLoss_mean_scfea': imbalanceLoss_mean_scfea, 'imbalanceLoss_mean_mpo': imbalanceLoss_mean_mpo,
           'imbalanceLoss_mean_snn': imbalanceLoss_mean_snn, 'imbalanceLoss_mean_mposnn': imbalanceLoss_mean_mposnn,
           'scale_mean_all_scfea': scale_mean_all_scfea, 'scale_mean_all_mpo': scale_mean_all_mpo,
           'scale_mean_all_snn': scale_mean_all_snn, 'scale_mean_all_mposnn': scale_mean_all_mposnn
           }
...
def plot_std_scale_imbalance_in_one(std_mean_scfea_all, std_mean_mpo_all, std_mean_snn_all, std_mean_mposnn_all, scale_mean_scfea_all,
                                    scale_mean_mpo_all, scale_mean_snn_all,scale_mean_mposnn_all,
    ...
    plt.plot(x, std_mean_mposnn_all, marker='*', label='mposnn')
    ...
    plt.plot(x, scale_mean_mposnn_all, marker='*', label='mposnn')
    plt.xlabel('iteration')
    ...
    plt.plot(x, imbalanceLoss_mean_mposnn_all, marker='*', label='mposnn')
    plt.xlabel('iteration')
