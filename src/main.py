...
def main(args):
    ...
    compounds_modules, modules_genes, geneExpression = update_compoundsModules_modulesGenes(compounds_modules, modules_genes, geneExpression)
    
    # plot the graph again after removing invalid modules and compounds
    if args.do_train_snn == 1 and args.do_predict_snn ==1:
        plot_FactorGraph(compounds_modules, title_name='compounds_modules_connected',save_path=args.output_dir+'compounds_modules_connected.png')
    ...
    minImbalance_flux_snn_epoch = -1
    cur_imbalanceLoss_mean_snn_all = float('inf')
    samples_modules_mpo_loss_min_target = []
    #loss_mean_min_mpo = float('inf')
    while (cur_step < args.n_epoch_all) or cur_imbalanceLoss_mean_snn_all < args.imbalance_loss_limit_all:
        print(SEP_SIGN)
        print("\n The {0}th epoch_all! \n".format(cur_step + 1))
        print(SEP_SIGN)
        ...
        if len(samples_modules_snn) == 0:
            remove_grad_files(minImbalance_flux_snn_epoch, args)
            continue
        ...
        if std_scale_imbalanceLoss['imbalanceLoss_mean_snn'] < minImbalance_flux_snn:
            minImbalance_flux_snn_epoch = cur_step
    ...
    #print("\n max_snn_step={0} \n".format(minImbalance_flux_snn_epoch))
    save_snn_model_weights(minImbalance_flux_snn_epoch, args)
    save_grad_file(args)
