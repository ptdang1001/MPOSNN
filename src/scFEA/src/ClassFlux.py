...
class FLUX(nn.Module):
    def __init__(self, matrix, n_modules, f_in = 50, f_out = 1): # there should be different input size.
        super(FLUX, self).__init__()
        # gene to flux
        self.inSize = f_in # ?? 500*n_modules,
    ...
    def updateC(self, m, n_comps, cmMat): # stoichiometric matrix, m is the number of module. m=[0,0,0.2,0,0,0.3,.......],m = flux values
        
        c = torch.zeros((m.shape[0], n_comps))
        for i in range(c.shape[1]):
            tmp = m * cmMat[i,:]# ??
            c[:,i] = torch.sum(tmp, dim=1)# compound_i = m_i+m_j-m_k.
