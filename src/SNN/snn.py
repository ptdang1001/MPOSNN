...
def predict(data, module_name, cur_step, args):
    ...
    if args.do_train_snn == 1 and args.do_predict_snn == 1:
        read_checkpoint_dir = args.load_checkpoints_dir + "SNN/model_weights_checkpoints/Epoch_" + str(cur_step).zfill(
            4) + '/' + module_name + "/"
    else:
        read_checkpoint_dir = args.load_checkpoints_dir + "SNN/final_model_weights/" + module_name + "/"
...
def snn(geneExpression, modules_genes, samples_modules_mpo, cur_step, args):
    ...
    if args.do_train_snn == 0 and args.do_predict_snn == 1:
        try:
        #if True:
            all_modules = list(modules_genes_train.keys())
            all_modules.sort()
            ...
            samples_modules_snn.columns = all_modules
            samples_modules_snn = samples_modules_snn.fillna(0.0).abs()
        except:
            print("\n Prediction Error!\n")
        # sys.exit(1)
    ...
    if args.do_train_snn == 1:
        ...
        save_checkpoint_dir = args.output_dir + "SNN/model_weights_checkpoints/" + "Epoch_" + str(cur_step).zfill(4) + '/'
        if os.path.exists(save_checkpoint_dir):
            shutil.rmtree(save_checkpoint_dir)

        # '''
        process_list = []
        for module_name in samples_modules_mpo_target.columns.values:
    ...
    if args.do_predict_snn == 1:
        ...
        try:
            ...
            samples_modules_snn =samples_modules_snn[samples_modules_mpo_target.columns]
            samples_modules_snn = samples_modules_snn.fillna(0.0).abs()
        except:
            print("\n Prediction Error! \n")
