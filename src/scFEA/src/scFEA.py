...
def scFEA(geneExpr,moduleGene,cmMat,args):
    ...
    '''
    geneExpr_path = "/N/slate/pdang/myProjectsDataRes/20211201FLUXOptimization/data/ccle_rna_quanti_normalized.csv"
    ...
    '''
    #moduleGene_dir = "/N/slate/pdang/myProjectsDataRes/20211201FLUXOptimization/data/module_info/"
    #moduleGene_file = "ant2_2_module_genes.json"
    #moduleGene = read_json2dict(moduleGene_dir,moduleGene_file)
    ...
    gene_overlap.sort()
    #moduleGene = {module_i: [gene for gene in moduleGene[module_i] if gene in gene_overlap] for module_i in modules}
    #moduleLen = np.array([len(moduleGene[module_i]) for module_i in modules])

    '''
    cmMat_path = "/N/slate/pdang/myProjectsDataRes/20211201FLUXOptimization/data/module_info/ant2_2_cmMat.csv"
    cmMat = pd.read_csv(
        #data_path + '/' + cm_file,
        #sep=',',
        #header=None
    ...
    if fileName == 'NULL':
        ...
        fileName = "./" + res_dir + "/" + test_file[-len(test_file):-4] + "_module" + str(n_modules) + "_cell" + str(
            n_cells) + "_batch" + str(BATCH_SIZE) + \
                   "_LR" + str(LEARN_RATE) + "_epoch" + str(EPOCH) + "_SCimpute_" + str(sc_imputation)[0] + \
                   "_lambBal" + str(LAMB_BA) + "_lambSca" + str(LAMB_NG) + "_lambCellCor" + str(
            LAMB_CELL) + "_lambModCor_1e-2" + \
                   '_' + timestr + ".csv"
    '''
    setF = pd.DataFrame(fluxStatuTest)
    #setF.columns = moduleGene.index
    setF.columns = modules
